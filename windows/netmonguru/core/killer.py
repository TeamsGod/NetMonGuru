...
class WinCutter:
    ...
    RULE = "NetMonGuru block "

    def __init__(self, runner: Runner = _run, keep_on_exit: bool = False,
    ...
    def available(self) -> bool:
        return is_admin() if self._admin is None else self._admin

    def _netsh(self, *args: str) -> Tuple[int, str]:
        return self.run(["netsh", "advfirewall", "firewall", *args], "")

    def set_blocked(self, ips: List[str]) -> Tuple[bool, str]:
        """Windows Firewall rules (in + out) per blocked host.  Firewall rules
        are persistent by nature; they are removed on exit unless
        ``keep_on_exit`` is set, and re-created from the blocklist on start."""
        ...
        for ip in [i for i in self.blocked if i not in wanted]:
            self._netsh("delete", "rule", f"name={self.RULE}{ip}")
        failed = ""
        for ip in [i for i in wanted if i not in self.blocked]:
            # start clean: a rule of ours may be left from a run that kept them
            self._netsh("delete", "rule", f"name={self.RULE}{ip}")
            for direction in ("out", "in"):
                code, out = self._netsh(
                    "add", "rule", f"name={self.RULE}{ip}",
                    f"dir={direction}", "action=block", f"remoteip={ip}",
                    "enable=yes", "profile=any")
                if code != 0:
                    failed = out.strip()[-80:] or f"netsh exit {code}"
        if failed:
            return False, f"netsh: {failed}"
        self.blocked = wanted
        return True, f"{len(wanted)} host(s) blocked (Windows Firewall)"
    ...
    def release(self) -> None:
        if self.keep_on_exit or not self.available:
            return
...
def make_cutter(keep_on_exit: bool = False):
    return WinCutter(keep_on_exit=keep_on_exit) if IS_WINDOWS \
        else PfCutter(keep_on_exit=keep_on_exit)
