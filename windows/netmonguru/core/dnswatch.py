...
MODES = ("auto", "log", "pcap", "etw", "cache", "passive", "off")
...
class DNSWatcher:
    ...
    def __init__(self, mode: str = "auto", window: int = DEFAULT_WINDOW,
        ...
        self._seen_cache: Dict[tuple, float] = {}
        self._pnames: Dict[int, str] = {}

    # -- lifecycle ---------------------------------------------------------
    @property
    def elevated(self) -> bool:
        from .win import is_admin
        return is_admin()
    ...
    def _pick_mode(self) -> str:
        ...
        if win.IS_WINDOWS:
            # etw: DNS Client operational log - knows the requesting process
            # cache: polling Get-DnsClientCache - no admin, no process
            wanted = self.requested_mode
            if wanted in ("auto", "etw", "log", "pcap"):
                if win.dns_log_enabled():
                    return "etw"
                if wanted == "etw" and self.elevated:
                    ok, why = win.enable_dns_log()
                    if ok:
                        return "etw"
                    self.error = f"could not enable the DNS log: {why}"
                return "cache"
            return wanted
    ...
    # -- windows sources ---------------------------------------------------
    def _windows_records(self, line: str) -> List[DNSRecord]:
        from . import win
        ...
        if not line:
            return []
        if self.mode == "etw":
            parsed = win.parse_dns_event_line(line)
            if parsed is None:
                return []
            name, rtype, answers, pid = parsed
            return [DNSRecord(ts=now, name=name, rtype=rtype, answers=answers,
                              source="etw", pid=pid or None,
                              client=self._pname(pid))]
        out = []
        for name, rtype, data, ttl in win.parse_dns_cache_line(line):
            # the cache is re-read every few seconds: report an answer once
            # per window, not on every poll
            key = (name, rtype, data[0])
            last = self._seen_cache.get(key)
            if last and now - last < self.cache.window:
                continue
            self._seen_cache[key] = now
            out.append(DNSRecord(ts=now, name=name, rtype=rtype, answers=data,
                                 ttl=ttl or None, source="cache"))
        if len(self._seen_cache) > 20000:
    ...
    def _pname(self, pid: int) -> str:
        ...
        if name is None:
            try:
                ...
                name = ""
            if len(self._pnames) > 2000:
                self._pnames.clear()
            self._pnames[pid] = name
        return name

    # -- capture loop ------------------------------------------------------
    def _command(self) -> List[str]:
        if self.mode in ("etw", "cache"):
            from . import win
            script = win.DNS_ETW_SCRIPT if self.mode == "etw" \
                else win.DNS_CACHE_SCRIPT
    ...
    def _run(self) -> None:
        ...
        try:
            for line in self._proc.stdout:
                ...
                try:
                    if self.mode in ("etw", "cache"):
                        records = self._windows_records(line)
                    elif self.mode == "log":
