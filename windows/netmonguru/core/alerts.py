...
def notify_windows(title: str, message: str, popen=subprocess.Popen) -> bool:
    """Toast through WinRT from Windows PowerShell - no extra package.  The
    toast is attributed to PowerShell's AppUserModelID because an unpackaged
    script has none of its own.  Fire and forget: PowerShell takes about half
    a second to start and the sampler must not wait for it."""
    from .win import NO_WINDOW, ps_quote

    script = TOAST_SCRIPT.format(title=ps_quote(title[:80]),
                                 message=ps_quote(message[:240]))
    try:
        popen(["powershell", "-NoLogo", "-NoProfile", "-NonInteractive",
...
def default_notifier() -> Callable[[str, str], bool]:
    from .win import IS_WINDOWS
    return notify_windows if IS_WINDOWS else notify_macos
