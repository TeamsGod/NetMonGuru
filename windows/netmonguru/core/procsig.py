...
class ProcSig:
    ...
    def short(self) -> str:
        return {"apple": "apple", "appstore": "store", "devid": "dev-id",
                ...
                "microsoft": "microsoft", "signed": "signed"}.get(
                    self.signing, "")
    ...
    def verdict(self) -> str:
        ...
        return "clean" if self.signing in ("apple", "appstore", "devid",
                                           "microsoft", "signed") else ""

    @property
    def signer(self) -> str:
        if self.signing == "apple":
            return "Apple system software"
        if self.signing == "microsoft":
            return "Microsoft" + (f" ({self.team})" if self.team else "")
...
class ProcSigner:
    ...
    def __init__(self, run=subprocess.run) -> None:
        ...
        self.force_windows = False          # tests exercise the Windows path
        self.ps_run = None                  # injectable PowerShell runner

    # -- Windows: Authenticode + Mark-of-the-Web ------------------------------
    def warm(self, exes) -> None:
        """Sign-check many binaries with ONE PowerShell start (it costs about
        half a second to launch, whatever the number of files)."""
        from . import win
        ...
        for exe in exes:
            try:
                ...
                continue
            if (exe, mtime) not in self._cache:
                todo.append((exe, mtime))
        if not todo:
            return
        kwargs = {"run": self.ps_run} if self.ps_run else {}
        result = win.authenticode([e for e, _ in todo], **kwargs)
        by_lower = {k.lower(): v for k, v in result.items()}
        for exe, mtime in todo:
            signing, publisher, issuer, note = by_lower.get(
                exe.lower(), ("unknown", "", "", "signature could not be "
                                                 "read"))
            self._cache[(exe, mtime)] = ProcSig(
                exe=exe, signing=signing, authority=publisher, team=issuer,
                path_flags=win.windows_path_flags(exe), error=note)

    def _inspect_windows(self, pid: Optional[int], exe: str, deep: bool
                         ) -> ProcSig:
        from . import win

        if not exe:
            return ProcSig(pid=pid, error="executable path not readable "
                                          "(protected process - run as "
                                          "Administrator)")
        try:
            mtime = os.path.getmtime(exe)
        except OSError:
            return ProcSig(pid=pid, exe=exe,
                           path_flags=win.windows_path_flags(exe)
                           + ["binary deleted from disk"])
        if (exe, mtime) not in self._cache:
            self.warm([exe])
        cached = self._cache.get((exe, mtime)) or ProcSig(exe=exe)
        if deep:
            if not cached.sha256:
                cached.sha256 = sha256_of(exe)
            if not cached.gatekeeper:
                cached.gatekeeper = win.mark_of_the_web(exe)
        fields = {k: v for k, v in cached.__dict__.items()
                  if not k.startswith("_")}
        return ProcSig(**{**fields, "pid": pid,
    ...
    def inspect(self, pid: Optional[int], exe: str, deep: bool = False
        ...
        if win.IS_WINDOWS or self.force_windows:
            return self._inspect_windows(pid, exe, deep)
        if not exe:
            return ProcSig(pid=pid, error="executable path not readable "
                                          "(needs sudo)")
        try:
