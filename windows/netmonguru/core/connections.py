...
class ConnectionCollector:
    ...
    def __init__(self, use_netstat: bool = True) -> None:
        ...
        # Windows ships a netstat.exe with a different output format; psutil
        # (GetExtendedTcpTable underneath) already returns every socket with
        # its owning pid there, so it is the only backend used.
        self._have_lsof = not IS_WINDOWS and shutil.which("lsof") is not None
        self._have_netstat = not IS_WINDOWS and \
            shutil.which("netstat") is not None
