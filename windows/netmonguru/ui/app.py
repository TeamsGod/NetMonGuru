...
from ..core.killer import make_cutter, terminate_process
from ..core.win import config_dir
...
LEVEL_TEXT = {"malicious": "bold #ff5f5f", "suspicious": "bold #ffaf5f",
              ...
              "": "#7a8a99"}
SIG_STYLES = {"microsoft": "#5f875f", "signed": "#5f875f",
              "apple": "#5f875f", "appstore": "#5f875f", "devid": "#5f875f",
...
class KillScreen(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        ...
        t.append("  — the kernel drops this exact socket pair at once;\n"
        ...
        t.append(f"  — ends it; closes all {self.sockets} of its socket(s) "
                 "at once\n", style="#7a8a99")
        t.append("  K  ", style="bold #ff5f5f" if can_sig else "#4a5a6a")
        t.append("kill process tree", style=style)
        t.append("  — also terminates its child processes (shift+k)\n",
                 style="#7a8a99")
        if not can_sig:
...
class NetMonGuruApp(App):
    ...
    def _render_detail_panel(self) -> None:
        ...
        if p:
            ...
            line(t, "process", "", extra="details need Administrator or the process "
                                         "has exited")
        else:
            line(t, "process", "", extra="owner unknown — run as Administrator to "
                                         "attribute sockets")
    ...
    def _render_processes(self) -> None:
        ...
        if not snap.procs:
            if not self.monitor.procnet.available:
                note.update("TCP statistics API not available — throughput columns are empty; "
                ...
                note.update("waiting for the first sample…")
        else:
            note.update("")
    ...
    def _render_proc_detail(self) -> None:
        ...
        if p:
            ...
            line(t, "executable", "", extra="details need Administrator or the process "
                                            "has exited")
        else:
            line(t, "executable", "", extra="owner unknown — run as Administrator to "
                                            "attribute sockets")
    ...
    def _render_intel_report(self, force: bool = False) -> None:
        ...
        if r is None:
            widget.update(Text(
                ...
                f" API keys: {config_dir() / 'keys.toml'}",
                style="#7a8a99"))
            return
